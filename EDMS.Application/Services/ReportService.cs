...
namespace EveryDaily.Application.Services
{
    public class ReportService : IReportService
    {
        ...
        public async Task<BaseResult<ReportDto>> CreateReportAsync(CreateReportDto createDto)
        {
            try
            {
                var user = await _userRepository.GetAll().FirstAsync(x => x.Id == createDto.userId);
                var report = await _reportRepository.GetAll().FirstAsync(x => x.Name == createDto.name);  
        ...
        public Task<BaseResult<ReportDto>> GetReportByIdAsync(long id)
        {
            ...
            baseResult = new BaseResult<ReportDto>()
            {
                Data = new ReportDto(report.Id, report.Name, report.Description, report.CreatedAt)
            };
